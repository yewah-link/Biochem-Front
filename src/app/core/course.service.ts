...
export class CourseService {
   ...
   * @returns Observable of the updated CourseDto
   */
  uploadThumbnail(courseId: number, file: File): Observable<CourseDto> {
    const formData = new FormData();
    formData.append('file', file);

    return this.http.post<GenericResponseV2<CourseDto>>(
  ...
  /**
   * Delete/remove the thumbnail from a course
   * @param courseId The ID of the course
   * @returns Observable of the updated CourseDto
   */
  deleteThumbnail(courseId: number): Observable<CourseDto> {
    return this.http.delete<GenericResponseV2<CourseDto>>(
      `${this.apiUrl}/${courseId}/delete-thumbnail`
    ).pipe(
      map(res => {
        if (res.status === 'SUCCESS' && res._embedded) {
          return res._embedded;
  ...
  getCourseThumbnailUrl(course: CourseDto): string {
    // If course has no ID or no thumbnail, return placeholder
    if (!course.id || !course.thumbnailUrl) {
      return 'assets/images/course-placeholder.png';
    }
    
    // Use the backend endpoint to serve the thumbnail
